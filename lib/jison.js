...
class LRGenerator extends LookaheadGenerator {
    ...
    generateModule_() {
        let parseSource = String(parser.parse);
    ...
    createParser() {
        const parser = new Function(this.generateModuleExpr() + 'return new Parser;')();
    ...
    afterparseTable() {
        ...
        if (this.conflicts > 0) {
            this.resolutions.forEach(function(r, i) {
                if (r[2].bydefault) {
                    self.warn('Conflict at state: ',r[0], ', token: ',r[1], "\n  ", printAction(r[2].reduce, self), "\n  ", printAction(r[2].shift, self));
                }
            });
...
parser.parse = function parse(input, ...parseArgs) {
