...
Jison.Parser = function(grammar, options) {
    var gen = createGenerator(grammar, options);
...
class Generator {
    ...
    processGrammar(grammar) {
        ...
        this.buildProductions(bnf, productions, nonterminals, this.symbols, this.operators);
        ...
        while(cont) {
            ...
            productions.forEach(function Follow_prod_forEach(production, k) {
                ...
                for (var i=0,t;t=production.handle[i];++i) {
                    ...
                    addMissed(nonterminals[t].follows, set);
        ...
        // RHS
            ...
            for (var i=0,t;t=symbol[i];++i) {
                if (!this.nonterminals[t]) {
                    ...
                    addMissed(firsts, this.nonterminals[t].first);
                }
                if (!this.nullable(t))
                    break;
            }
        ...
        while(cont) {
            ...
            for (symbol in nonterminals) {
                ...
                nonterminals[symbol].productions.forEach(function(production) {
                    addMissed(firsts, production.first);
                });
...
class LRGenerator extends LookaheadGenerator {
    ...
    parseTable(itemSets) {
            ...
            for (const reduction of itemSet.reductions) {
                ...
                for (const stackSymbol of terminals) {
                    ...
                    if (action && action.length) {
                        ...
                        action = [r, reduction.production.id];
                    }
        ...
        if (!this.DEBUG && this.conflicts > 0) {
            ...
            for (const state of Object.keys(conflictedStates)) {
...
function resolveConflict(production, op, reduce, shift) {
    var sln = { production, operator: op, r: reduce, s: shift },
        s = 1, // shift
        r = 2, // reduce
        a = 3; // accept
...
class LALRGenerator extends LRGenerator {
    ...
    constructor(grammar, options) {
        ...
        var newg = this.newg = Object.assign(Object.create(LookaheadGenerator.prototype), {
            oldg: this,
            trace: this.trace,
            nterms_: {},
            DEBUG: false,
    ...
    parseTable(productions) {
        ...
        productions.forEach(function(production, i) {
            ...
            if (self.nullable(production.handle)) {
                addMissed(tokens, self.nonterminals[production.symbol].follows);
            }
...
function createGenerator(g, options) {
    const opt = { ...g.options, ...options };

    switch (opt.type) {
        case 'lr0': return new LR0Generator(g, opt);
        case 'slr': return new SLRGenerator(g, opt);
        case 'lr':
        case 'lr1': return new LR1Generator(g, opt);
        case 'll':  return new LLGenerator(g, opt);
        case 'lalr':
        default:    return new LALRGenerator(g, opt);
    }
}
